...
> 1. Eine *Homotopie* von $f \text{ nach } g \text{ relativ zu } M$ ist eine stetige Abbildung $h: [0,1] \times X \to Y, (t,x) \mapsto h(t,x)$ mit $h(0,x) = f(x), h(1,x) = g(x), h(t,m) = f(m) = g(m)$ $\forall x \in X, m \in M, t \in [0,1]$.
